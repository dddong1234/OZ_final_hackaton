...
이 단계는 LightGBM 튜닝이 아니라 전처리 효과가 비선형 모델에서도 유지되는지
확인하는 실험이다. seed 42에서 baseline을 의미 있게 개선한 후보만 LightGBM
3-seed confirmation 대상으로 올린다.

## 7. 반복 검증 후 연구 후보

상위 후보가 안정적일 때만 다음 조합을 별도 실험으로 진행한다.
